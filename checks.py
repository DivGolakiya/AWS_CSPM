...
def validate_credentials(aws_access_key_id, aws_secret_access_key):
    ...
    try:
        sts_client = get_boto_client('sts', aws_access_key_id, aws_secret_access_key)
        sts_client.get_caller_identity()
        return True, None
    except ClientError as e:
        if e.response['Error']['Code'] == 'InvalidClientTokenId':
            return False, "The AWS Access Key ID you provided does not exist or is not valid."
        elif e.response['Error']['Code'] == 'SignatureDoesNotMatch':
            return False, "The AWS Secret Access Key you provided is incorrect."
        else:
            return False, f"An authentication error occurred: {e.response['Error']['Code']}"

def get_available_regions(aws_access_key_id, aws_secret_access_key):
    """Gets a list of all available AWS regions for the account."""
    try:
...
# --- UPDATED & EXISTING CHECK FUNCTIONS ---
def check_s3_public_access(aws_access_key_id=None, aws_secret_access_key=None, quiet=False):
    ...
    if not quiet: print(f"  [+] S3 check complete. Found {len(findings)} issues.")
    return findings

def check_open_security_groups(aws_access_key_id=None, aws_secret_access_key=None, regions_to_scan=None, quiet=False):
    if not quiet: print("[*] Checking for open security groups...")
    findings = []
    sensitive_ports = [22, 3389]
    if not regions_to_scan:
        regions_to_scan = get_available_regions(aws_access_key_id, aws_secret_access_key)
        if not regions_to_scan:
             return [{"resource": "EC2 Service", "finding": "Could not list AWS regions.", "recommendation": "Ensure credentials have ec2:DescribeRegions permission."}]
    ...
    for region in regions_to_scan:
        try:
            ...
            if not quiet: print(f"  [!] Warning: Could not scan region {region}. It may be disabled or invalid.")
            continue
    if not quiet: print(f"  [+] Security group check complete. Found {len(findings)} issues.")
    return findings
...
def check_root_mfa(aws_access_key_id=None, aws_secret_access_key=None, quiet=False):
    ...
    if not quiet: print(f"  [+] Root MFA check complete. Found {len(findings)} issues.")
    return findings
                ...
                try:
                    ...
                    if last_used_date is None:
                     ...
                     if not quiet and e.response['Error']['Code'] == 'AccessDenied':
                        print(f"  [!] Warning: Could not check last use for key {key_id}.")
                     continue
    except ClientError as e:
        return [{"resource": "IAM Service", "finding": f"Could not list IAM users. Error: {e.response['Error']['Code']}", "recommendation": "Ensure credentials have iam:ListUsers, iam:ListAccessKeys, and iam:GetAccessKeyLastUsed permissions."}]
    if not quiet: print(f"  [+] Inactive IAM key check complete. Found {len(findings)} issues.")
    return findings
...
def check_cloudtrail_enabled(aws_access_key_id=None, aws_secret_access_key=None, quiet=False):
        ...
        for trail in response['trailList']:
            if trail.get('IsMultiRegionTrail'):
                ...
                if trail_status.get('IsLogging'):
                    active_multi_region_trail = True; break
    ...
    if not quiet: print(f"  [+] CloudTrail check complete. Found {len(findings)} issues.")
    return findings
...
def check_iam_password_policy(aws_access_key_id=None, aws_secret_access_key=None, quiet=False):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'NoSuchEntity':
            findings.append({"resource": "IAM Password Policy", "finding": "No IAM password policy is configured for the account.", "recommendation": "Create a strong IAM password policy."})
        else:
    ...
    if not quiet: print(f"  [+] IAM password policy check complete. Found {len(findings)} issues.")
    return findings
...
def check_publicly_accessible_rds(aws_access_key_id=None, aws_secret_access_key=None, regions_to_scan=None, quiet=False):
    ...
    for region in regions_to_scan:
        try:
            ...
            if not quiet: print(f"  [!] Warning: Could not scan region {region}. It may be disabled or invalid.")
            continue
    if not quiet: print(f"  [+] Public RDS check complete. Found {len(findings)} issues.")
    return findings
